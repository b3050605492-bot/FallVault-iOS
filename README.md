...
**验证码（TOTP）**
- 标准 RFC 6238 实现（base32 → HMAC → 动态截断），与 Google Authenticator、桌面版 **完全同码**
- 密钥格式全兼容：纯 Base32 密钥、`otpauth://` 链接、**`otpauth-migration://` 链接**（Google Authenticator 迁移格式，直接解析 protobuf 取出真实密钥）
- 算法参数自动识别：SHA-1 / SHA-256 / SHA-512、6 位 / 8 位、30 秒 / 自定义周期
- 时间偏移：**联网自动校准**（淘宝 / 苏宁 / 世界时间多源依次尝试），也可手动微调；设置本地持久化，重开应用仍生效
- 倒计时圆环、点击复制；支持 `otpauth://` 链接导入 / 导出

**银行卡卡片**
- 卡面配色 / 自定义卡面图（上传后可裁剪、缩放、拖动调整；超大原图自动安全缩放）
- 卡号分组显示、CVV 隐藏、账单地址（地址 / 城市 / 省州 / 国家地区 / 邮编，选填）
- 上下滑动切换的 3D 卡片画廊

**隐私与安全**
- 锁屏 / 主界面壁纸：内置多张 + 上传自己的图（可裁剪、缩放、拖动调整），**设置持久保存，重开应用仍生效**
...
> **恢复是「合并」不是「覆盖」**：恢复备份时，重复的条目会被跳过（按 标题+账号+密码+网址 判定），已有数据不会被清空 —— 连续恢复多份备份是累加关系，与桌面版行为一致。
>
> **银行卡数据会随备份往返**：iOS 的银行卡信息会写进 `.fvault` 备份；桌面版没有卡片界面，但恢复时会把卡片**原样保管**，再次导出时带回来 —— 所以「iOS → 桌面版 → iOS」这一圈不会丢卡片。
...
> **直接下载**：[FallVault.ipa（最新版）](https://github.com/b3050605492-bot/FallVault-iOS/releases/latest) —— 全部版本见 [Releases](https://github.com/b3050605492-bot/FallVault-iOS/releases)
